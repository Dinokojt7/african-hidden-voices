...
export default function Nav() {
  ...
  return (
    <>
      {isHome && (
        <div style={{ background: "var(--ahv-oxblood-deep)", color: "var(--ahv-cream)", textAlign: "center", fontFamily: "var(--font-hanken)", fontSize: "12.5px", letterSpacing: ".14em", textTransform: "uppercase", padding: "9px 20px", borderBottom: "1px solid rgba(201,162,75,.28)" }}>
          <span style={{ color: "var(--ahv-gold)" }}>◆</span>&nbsp;&nbsp;In partnership with the University of Fort Hare &nbsp;·&nbsp; Indigenous Knowledge Systems Conference 2025
        </div>
      )}
      <nav style={{ position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 40px", transition: "background .35s ease, box-shadow .35s ease, border-color .35s ease", borderBottom: "1px solid transparent", ...barStyle }}>
        <Link href="/" style={{ display: "flex", alignItems: "center" }}>
          <Image src="/logo.png" alt="African Hidden Voices" width={180} height={52} style={{ height: 52, width: "auto", display: "block", mixBlendMode: "multiply" }} />
        </Link>

        <div style={{ display: "flex", alignItems: "center", gap: 26, fontFamily: "var(--font-hanken)", fontSize: "13.5px", fontWeight: 600, letterSpacing: ".04em", color: fg }} className="hidden md:flex">
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu" style={{ background: "transparent", border: `1.5px solid ${fg}`, borderRadius: 8, width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: fg }} className="flex md:hidden">
